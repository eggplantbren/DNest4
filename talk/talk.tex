...
I divide MCMC (or similar) sampling strategies into two classes.
The first category focuses on {\bf how to move around}:
e.g., Metropolis, Gibbs sampling, Slice Sampling, Hamiltonian MCMC,
Affine Invariant Ensemble Sampler.\\[0.5em]\pause

The second category {\bf modifies the target distribution}.
Examples: Parallel tempering (and other methods based on `annealing'
or `power posteriors'), Nested Sampling, Diffusive Nested Sampling.
...
\begin{frame}
\frametitle{Mixture of Two Gaussians}
Taken from John Skilling's 2000s Nested Sampling papers.
$\pi(\bx)$ is Uniform$(-1, 1)$ in 20 dimensions. $L(\bx)$ is a mixture
of a broad and a narrow gaussian:

\begin{align}
L(\bx) &= \prod_{i=1}^{20}
            \frac{\exp\left(-\frac{1}{2}(x_i/v)^2\right)}{v\sqrt{2\pi}}
        + 100 \prod_{i=1}^{20}
            \frac{\exp\left(-\frac{1}{2}(x_i/u)^2\right)}{u\sqrt{2\pi}}.
\end{align}
With $u=0.01$ and $v=0.1$ the narrow peak dominates the posterior (but not
entirely).
