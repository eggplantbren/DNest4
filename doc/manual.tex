...
I have only ever compiled \dnest~on GNU/Linux,
...
It should be possible, but probably more tricky,
...
has tried this, and any problems that arose.
I would also be interested to hear if anyone has
compiled \dnest~with a different compiler, such as the Intel C++
compiler.\\

\subsection{Dependencies}
One of the issues with DNest3 (an older implementation) which inspired a rewrite
was that many users
...
{\tt NumPy}, {\tt matplotlib}, {\tt Pandas}, and {\tt Cython}.
They should work with both Python 2 or 3.

\subsection{Compiling}
Go into the {\tt code} directory and run {\tt make}. Everything should compile.
To
verify that the compilation was successful,
go into the {\tt code/Examples/SpikeSlab} directory
...
the C++ parts of \dnest~were compiled successfully.\\

Next, you'll need to install the {\tt dnest4} Python package so you can use
the postprocessing scripts, and other features. Go into the
{\tt python} directory and run {\tt python setup.py install} or similar.

\section{Running DNest4}
...
To run the SpikeSlab example, go into the {\tt code/Examples/Spikeslab}
directory and execute {\tt main}. You should see some output that looks like
that shown in Figure~\ref{fig:output}.
You must terminate the process manually by pressing Ctrl+C
or in another way. If you do not terminate the process yourself, it will run
forever. There is no definite rule for how long you should run \dnest. Like
many MCMC methods, it depends on how hard your problem is, the number of
posterior samples you
want, and the accuracy you want for the marginal likelihood or other
...
are 20 columns in {\tt sample.txt}.
