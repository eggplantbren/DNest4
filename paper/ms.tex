...
\end{align}
The specific assumptions of \pkg{RJObject} are
\begin{align}
p\left(N, \alpha, \left\{\boldsymbol{x}_i\right\}_{i=1}^N\right)
&= p(N)p(\alpha)p\left(\left\{\boldsymbol{x}_i\right\}_{i=1}^N | \alpha\right).
\end{align}
which is depicted in Figure~\ref{fig:rjobject_pgm}.

\begin{figure}[ht!]
\begin{center}
\includegraphics[width=0.4\textwidth]{figures/rjobject_pgm.pdf}
\caption{\label{fig:rjobject_pgm}}
\end{center}
\end{figure}
