...
{\tt code/Examples/StraightLine} subdirectory. A simplified version of the
code is explained in Section~\ref{sec:models}.

To execute \pkg{DNest4} on this problem, run the executable {\tt main}
in the {\tt code/Examples/StraightLine} directory. The output to the screen
...
\section{Output files}
The output files themselves aren't particularly useful. The goal of running
NS is usually to obtain posterior samples (which, again, {\tt sample.txt}
{\em does not provide}) and the marginal likelihood $Z$. Additional
...
\end{align}
and the number of saved particles with significant posterior weight.
...
execute a run. Here is the OPTIONS file used in the linear regression
example.
...
10000   # save interval
100     # threadSteps - how many steps each thread should do independently before pooling information
0       # maximum number of levels (0 = auto)
...
The first option is the number of particles per thread, here set to one.
If you use more particles
