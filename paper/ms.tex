...
(ii)
A \pkg{Python} package allowing basic use without \proglang{C++} coding; and
iii)
...
The software can be obtained from the \pkg{git} repository located at
\begin{center}
{\tt https://github.com/eggplantbren/DNest4/}\\
\end{center}
and is licensed under the permissive open source
...
if the templates from the \pkg{DNest4} repository are copied
...
the levels in column 1), as discussed in Section 3 of \citet{dnest}. The visits column counts the number of times a level (level $j$, say)
...
from \citet{rjobject} are also included in the \pkg{DNest4} repository.
...
using this for model selection \citep{robert}.
...
\bibitem[Giffin and Caticha(2007)]{giffin} Giffin, A., Caticha, A.\ 2007.\ Updating Probabilities with Data and Moments.\ Bayesian Inference and Maximum Entropy
...
\bibitem[Robert et al.(2011)]{robert}
Robert, C.P., Cornuet, J.M., Marin, J.M. and Pillai, N.S., 2011. Lack of confidence in approximate Bayesian computation model choice. Proceedings of the National Academy of Sciences, 108(37), pp.15112-15117.
